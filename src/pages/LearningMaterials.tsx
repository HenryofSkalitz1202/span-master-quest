	...
	const fetchSummary = async () => {
		...
		try {
			...
			if (!res.ok) {
				const err = await res.json().catch(() => ({}));
				throw new Error(
					err?.error || `Gagal ringkas (HTTP ${res.status})`
				);
			}
			...
			setExplainLeft((s) => {
				if (s <= 1) {
					if (explainRef.current)
						window.clearInterval(explainRef.current);
					handleNext(false);
					return 0;
				}
				return s - 1;
	...
	const renderOption = (text: string, idx: number) => {
		...
		return (
			...
			>
				...
				{showCorrect && (
					<Check className="ml-auto mt-1 h-5 w-5 text-green-600" />
				)}
				{showWrong && (
					<X className="ml-auto mt-1 h-5 w-5 text-red-600" />
				)}
			</button>
		);
	};
							...
							<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
								...
								<div className="flex items-end gap-2">
									<input
										...
										className="h-4 w-4"
									/>
									<Label htmlFor="include_explanation">
										Sertakan penjelasan
									</Label>
								</div>
							</div>
					...
					<Card className="overflow-hidden">
						...
						<CardContent>
							<div className="flex flex-wrap items-center gap-3 text-sm text-muted-foreground mb-4">
								...
								<span>
									• Tingkat: <b>{difficulty}</b>
								</span>
								<span>
									• Bahasa: <b>{language.toUpperCase()}</b>
								</span>
								<span>
									• Estimasi: ~
									{Math.ceil(
										(questions.length * MAX_TIME) / 60
									)}{" "}
									menit
								</span>
							</div>
