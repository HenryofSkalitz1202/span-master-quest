...
function clampPath(path: [number, number][], grid: number) {
  return path.map(
    ([r, c]) =>
      [Math.max(0, Math.min(grid - 1, r)), Math.max(0, Math.min(grid - 1, c))] as [number, number]
  );
}
...
function canAddToken(tokens: Token[], t: Token, openParens: number) {
  ...
  if (t.type === "paren") {
    if (t.value === "(") {
      ...
      if (openParens > 0 && L && (isNum(L) || (isParen(L) && L.value === ")"))) return true;
      return false;
    }
  }
  return false;
...
export default function NumericalChallenge({ onComplete, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    if (it.variant === "target_24") {
      const nums: number[] = it.render?.numbers || it.metadata?.numbers || [];
  ...
  const nums24: number[] = (it?.variant === "target_24")
  ...
  function addNumber(idx: number) {
    ...
    const t: NumToken = { type: "num", value: String(nums24[idx]), srcIndex: idx };
    setTokens((prev) => {
      const open = recomputeOpenParens(prev);
      if (!canAddToken(prev, t, open)) return prev;
  ...
  const usesAll = it?.variant === "target_24"
    ? usesExactly(expr24, nums24)
    : false;
  const eqTarget = it?.variant === "target_24"
    ? (evalVal !== null && Math.abs(evalVal - (it?.render?.target || 24)) < 1e-6)
    : false;
  ...
  function submit(auto = false) {
    ...
    if (cur.variant === "target_24") {
      ...
      try {
        ...
        ok =
          ...
          usesExactly(expr24, nums24) &&
          openParens === 0 &&
          (isNum(last(tokens)) || (isParen(last(tokens)) && last(tokens)?.value === ")"));
      } catch {
        ok = false;
      }
    }
    ...
    if (cur.variant === "equation_fill") {
      ...
      try {
        ok = idx === blanks && Math.abs(safeEval(L) - safeEval(R)) < 1e-9;
      } catch {
        ok = false;
      }
    }
    ...
    if (cur.variant === "function_machine") {
      ...
      if (Number.isFinite(want)) {
        try {
          ...
          const x = Number((cur.render?.query as string)?.match(/\(([-\d.]+)\)/)?.[1] || "NaN");
    ...
    if (["modular_arith", "base_convert", "prob_ratio"].includes(cur.variant)) {
      const want = Number(free);
      if (Number.isFinite(want)) {
        if (cur.variant === "base_convert") {
          ...
          const nums = (cur.prompt.match(/(\d+)\s*×\s*(\d+)\s*\+\s*(\d+)/) || [])
            .slice(1)
            .map(Number);
          ok = nums.length === 3 && Number.isFinite(m) && want === ((nums[0] * nums[1] + nums[2]) % m);
        } else if (cur.variant === "prob_ratio") {
          const [num, den] = (cur.metadata?.fraction as [number, number]) || [0, 1];
  ...
  if (!items.length) return <div className="p-6">Tidak ada soal.</div>;
  ...
  return (
    <div className="container max-w-3xl mx-auto py-6">
      ...
      {it?.variant === "target_24" && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Gunakan <b>semua</b> angka berikut tepat sekali. Target:{" "}
              <b>{it.render?.target || 24}</b> • Slot tersedia: {it.render?.slots || 7}
            </div>
            ...
            <div className="p-3 border rounded-lg bg-muted/40">
              <div className="flex flex-wrap gap-2 min-h-[40px]">
                {tokens.length === 0 && (
                  <span className="text-muted-foreground">Bangun ekspresimu di sini…</span>
                )}
                {tokens.map((t, i) => (
                  <button
                    key={i}
                    className={`px-2.5 py-1.5 rounded-md text-sm border bg-background hover:bg-muted transition ${
                      t.type === "num" ? "font-semibold" : "opacity-90"
                    }`}
                    onClick={() => removeAt(i)}
              ...
              <div className="mt-3 text-sm">
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="font-mono break-all">{compose(tokens) || "…"}</span>
                  <span className="text-muted-foreground">•</span>
                  <span>
                    {openParens === 0 ? "()" : `(${openParens} terbuka)`}
                  </span>
                  <span className="text-muted-foreground">•</span>
                  {(() => {
                    if (!isAns) return <span>Memorize dulu…</span>;
                    ...
                    if (expr24 && evalErr) return <span className="text-red-500">Ekspresi tidak valid</span>;
                    return <span>—</span>;
                  })()}
                  <span className="text-muted-foreground">•</span>
                  <span className={`${usesAll ? "text-emerald-600" : "text-amber-600"}`}>
                    {usesAll ? "Angka terpakai semua" : "Angka belum terpakai semua"}
                  </span>
                  {evalVal !== null && (
                    <>
                      <span className="text-muted-foreground">•</span>
                      <span className={`${eqTarget ? "text-emerald-600" : "text-rose-600"}`}>
                        {eqTarget ? "✅ Tepat sasaran!" : "Belum sama target"}
                      </span>
                    </>
                  )}
                </div>
            ...
            {/* Angka chips */}
            <div>
              <div className="text-xs text-muted-foreground mb-1">Angka</div>
            ...
            <div>
              <div className="text-xs text-muted-foreground mb-1">Operator & Tanda Kurung</div>
              <div className="flex flex-wrap gap-2">
                {(["+", "-", "×", "÷"] as const).map((op) => (
                  <Button
                    key={op}
                ...
                <Button size="sm" variant="secondary" onClick={() => addParen("(")} disabled={isMem}>
                  (
                </Button>
                <Button size="sm" variant="secondary" onClick={() => addParen(")")} disabled={isMem}>
                  )
                </Button>
                <Button size="sm" variant="outline" onClick={backspace} disabled={isMem}>
                  ⌫
                </Button>
                <Button size="sm" variant="outline" onClick={clearAll} disabled={isMem}>
                  Clear
                </Button>
              </div>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="mb-2 text-sm text-muted-foreground">
              Klik jalur {it.render?.maxSteps || 4} langkah dari sel awal.
            </div>
            <div
              className="grid"
              style={{ gridTemplateColumns: `repeat(${it.render?.grid || 3}, minmax(0, 1fr))`, gap: 8 }}
            >
              {(it.render?.cells || []).flatMap((row: number[], r: number) =>
                row.map((val: number, c: number) => {
                  const start = toTuple2(it.render?.start);
                  const pathCells: [number, number][] = [start, ...path];
                  const lastPt = pathCells[pathCells.length - 1];
                  const here = pathCells.findIndex((p) => p[0] === r && p[1] === c);
                  const isStart = r === start[0] && c === start[1];
                  const canStep =
                    Math.abs(lastPt[0] - r) + Math.abs(lastPt[1] - c) === 1 &&
                    path.length < (it.render?.maxSteps || 4) &&
                    isAns;
                  return (
                    <button
                      key={`${r}-${c}`}
                      className={`h-14 rounded-lg border flex items-center justify-center ${
                        isStart ? "bg-primary/10 border-primary" : "bg-muted"
                      } ${here >= 0 ? "ring-2 ring-primary" : ""} ${
                        canStep ? "hover:bg-primary/5" : "opacity-70"
                      }`}
                      onClick={() => {
                        if (canStep) setPath((p) => [...p, [r, c]]);
                      }}
                      disabled={isMem}
                    >
                      {val}
            ...
            <div className="mt-3 text-sm">
              {(() => {
                const start = toTuple2(it.render?.start);
                const pts: [number, number][] = [start, ...path];
                return <>Jalur: {pts.map((p) => `(${p[0]},${p[1]})`).join(" → ")}</>;
              })()}
            </div>
            <Button className="mt-3" variant="outline" onClick={() => setPath([])} disabled={isMem}>
              Reset Jalur
            </Button>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="text-xl font-mono">
              {it.render?.expressionLeft} = {it.render?.expressionRight}
            </div>
            <div className="flex gap-2">
              {Array.from({ length: it.render?.blanks || 2 }, (_, i) => (
                <button
                  key={i}
                  className={`h-10 w-12 border rounded-md text-lg font-mono ${
                    focusBlank === i ? "ring-2 ring-primary" : ""
                  } ${isMem ? "opacity-70" : "bg-background"}`}
            ...
            <div className="grid grid-cols-5 gap-2 max-w-xs">
              {["1","2","3","4","5","6","7","8","9","0"].map((d) => (
                <Button key={d} size="sm" onClick={() => typeDigit(d)} disabled={isMem}>
                  {d}
                </Button>
              ))}
              <Button size="sm" variant="outline" onClick={eraseDigit} disabled={isMem}>
                ⌫
              </Button>
              <Button size="sm" variant="outline" onClick={clearDigits} disabled={isMem}>
                Clear
              </Button>
            </div>
          </CardContent>
      ...
      {["function_machine", "modular_arith", "base_convert", "prob_ratio"].includes(it?.variant || "") && (
        <Card>
          <CardHeader>
            <CardTitle>{it?.prompt}</CardTitle>
          </CardHeader>
