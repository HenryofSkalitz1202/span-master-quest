...
import { PAD, CELL, getMarkerCellFromSvg, getLandmarkCentersFromSvg, gridToPixelCenters, rotateGridPoints, sameCenters } from "./shared";
...
function line(x1:number,y1:number,x2:number,y2:number,stroke="#1f2937",sw=1){return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${stroke}" stroke-width="${sw}" />`;}
function circle(cx:number,cy:number,r:number,fill:string){return `<circle cx="${cx}" cy="${cy}" r="${r}" fill="${fill}" />`;}
function rect(x:number,y:number,w:number,h:number,fill:string){return `<rect x="${x}" y="${y}" width="${w}" height="${h}" rx="4" ry="4" fill="${fill}" />`;}

function renderBaseMapSvg(grid:number, base:any, opts?:{marker?:[number,number], axis?:any}) {
  ...
  const rv = base?.river || [];
  if (rv.length >= 2){
    const pts = rv.map(([r,c]:[number,number]) => `${pad + c*CELL + CELL/2},${pad + r*CELL + CELL/2}`).join(" ");
  ...
  (base?.landmarks || []).forEach((lm:any) => {
    ...
    else if (icon === "triangle") parts.push(`<polygon points="${cx},${cy-14} ${cx-12},${cy+10} ${cx+12},${cy+10}" fill="${col}" />`);
...
export default function SpatialChallenge({ onComplete, onBack }: Props) {
  ...
  function check(): boolean {
    ...
    if (it.variant === "route_nav") {
      ...
      const startPos = (it.render?.base?.landmarks || []).find((lm: any) => lm.name === startName)?.pos || [0, 0];

      const dir: Record<string, [number, number]> = { N: [-1, 0], S: [1, 0], E: [0, 1], W: [0, -1] };
      let [r, c] = startPos;
    ...
    if (it.variant === "mirror_reflect") {
      ...
      const target = (it.render?.base?.landmarks || []).find((lm: any) => lm.name === "Pasar") || (it.render?.base?.landmarks || [])[0];
      ...
      const correct = (it.options as ChallengeOption[])?.find(o => {
        const cell = getMarkerCellFromSvg(o.render?.svg || "", grid);
        return cell && cell[0] === reflected[0] && cell[1] === reflected[1];
      })?.optionId;
    ...
    if (it.variant === "map_rotate") {
      ...
      const baseLms: [number, number][] = (it.render?.base?.landmarks || []).map((lm: any) => [lm.pos[0], lm.pos[1]]);
      const centersExpected = gridToPixelCenters(rotateGridPoints(baseLms, grid, it.render?.action?.deg || 90));

      const correct = (it.options as ChallengeOption[])?.find(o => {
        const centersGot = getLandmarkCentersFromSvg(o.render?.svg || "");
  ...
  if (it?.render?.base) {
    if (it.variant === "route_nav") {
      const startName = it.render?.action?.from;
      const startPos = (it.render?.base?.landmarks || []).find((lm: any) => lm.name === startName)?.pos || [0, 0];
      baseSvg = renderBaseMapSvg(it.render?.grid ?? 5, it.render?.base, { marker: startPos });
    } else if (it.variant === "mirror_reflect") {
      baseSvg = renderBaseMapSvg(it.render?.grid ?? 5, it.render?.base, { axis: it.render?.action?.axis });
    } else { // map_rotate
  ...
  return (
    <div className="container max-w-3xl mx-auto py-6">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="text-sm text-muted-foreground">Soal {ix + 1} / {items.length} • Skor: {score}</div>
      </div>
        ...
        <CardContent>
          {phase === "memorize" ? (
            <div className="rounded-lg border overflow-hidden">
              <div className="p-2 text-sm font-medium">Peta dasar (hafalkan)</div>
              <Separator />
              <div className="p-2" dangerouslySetInnerHTML={{ __html: baseSvg }} />
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-4">
              {(it.options as ChallengeOption[])?.map(o => (
                <button
                  key={o.optionId}
                  className={`rounded-lg border overflow-hidden hover:shadow ${selected === o.optionId ? "ring-2 ring-primary" : ""}`}
                  onClick={() => setSelected(o.optionId!)}
                >
                  <div className="p-2 text-left text-sm font-medium">Opsi {o.optionId}</div>
                  <Separator />
                  <div className="p-2" dangerouslySetInnerHTML={{ __html: o.render?.svg || "" }} />
                </button>
              ))}
            </div>
          )}
      ...
      <div className="flex justify-end gap-2 mt-4">
        <Button variant="secondary" onClick={() => (phase === "memorize" ? undefined : next())} disabled={phase === "memorize"}>Lewati</Button>
        <Button onClick={submit} disabled={phase === "memorize"}>Kirim Jawaban</Button>
      </div>
    </div>
  );
}
