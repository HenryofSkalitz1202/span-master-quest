...
export default function MemoryChallenge({ onComplete, onBack }: Props) {
	...
	function submit(auto = false) {
		...
		if (it.variant === "lexicon_match") {
			...
			ok =
				pairs.length > 0 &&
				pairs.every((p) => mapping[p.term] === p.definition);
		}
		...
		if (it.variant === "sequence_missing") {
			...
			ok =
				mask.length > 0 &&
				mask.every((m, j) => Number(seqInputs[j]) === Number(seq[m]));
		}
					...
					<CardContent>
						{phase === "memorize" ? (
							<div className="flex flex-wrap gap-2">
								{(it.render?.sequence || []).map(
									(n: number, i: number) => (
										<div
											key={i}
											className="px-3 py-2 rounded-lg border bg-muted"
										>
											{n}
										</div>
									)
							...
							<div className="flex flex-wrap gap-2">
								{(it.render?.sequence || []).map(
									(n: number, i: number) => {
										const mask = (it.render?.maskIndices ||
											[]) as number[];
				...
				<Card>
					...
					<CardContent>
								...
								{Array.from(
									...
									(_, k) => {
										...
										return (
											<div
												key={k}
												className="h-12 w-12 border rounded-md flex items-center justify-center bg-muted/40"
											>
												{obj
													? ICONS[obj.icon] || "⬛"
													: ""}
						...
						{phase === "answer" && (
							<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
								{(
									(it.answerSpec?.options ||
										it.options ||
										[]) as string[]
								).map((opt) => (
									<button
										key={opt}
										className={`p-3 border rounded-lg text-left hover:shadow ${
											selectedText === opt
												? "ring-2 ring-primary"
												: ""
										}`}
