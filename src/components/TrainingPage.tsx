...
const PREVIEWS: Record<ChallengeId, Preview> = {
	memory: {
		title: "Contoh: Urutan Angka",
		sample: "3 • 8 • 1 • 6 → ulangi urutan.",
		hint: "Gunakan chunk (38 | 16).",
	},
	spatial: {
		title: "Contoh: Pola & Rotasi",
		sample: "Pilih bentuk setelah diputar 90°.",
		hint: "Cari sudut/sisi jangkar.",
	},
	numerical: {
		title: "Contoh: Aritmetika Cepat",
		sample: "27 × 4 – 18 ÷ 3 = ?",
		hint: "Prioritaskan operasi, jaga ritme.",
	},
};
...
export default function TrainingPage() {
	...
	const [showPreviewFor, setShowPreviewFor] = useState<ChallengeId | null>(
		null
	);
	...
	const dailyProgress = Math.min(
		100,
		Math.round(
			(Math.min(dailyXPToday, DAILY_GOAL_XP) / DAILY_GOAL_XP) * 100
		)
	);
	const nav = useNav();
		...
		() => [
			{
				...
				description: "Ingat urutan angka",
				icon: Brain,
				difficulty: "Mudah",
				estimatedTime: "5–10 m",
				color: "from-blue-400 to-blue-600",
			},
			{
				id: "spatial" as const,
				title: "Tantangan Spasial",
				description: "Pola & ruang",
				icon: Compass,
				difficulty: "Sedang",
				estimatedTime: "8–15 m",
				color: "from-green-400 to-green-600",
			},
			{
				id: "numerical" as const,
				title: "Tantangan Numerik",
				description: "Hitung & logika",
				icon: Calculator,
				difficulty: "Menantang",
				estimatedTime: "10–20 m",
	...
	const handleComplete = (rawScore: number) => {
		...
		const bonus =
			new Date().getDate() % 3 ===
			["memory", "spatial", "numerical"].indexOf(selected)
				? 2
				: 1; // sama spt dailyChallenge
	...
	return (
		<TooltipProvider>
			<div className="min-h-screen bg-background md:p-6 mt-44 md:mt-20">
						...
						<div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
							<div>
								...
								<p className="text-muted-foreground">
									Bangun fokus & kecepatan berpikir lewat sesi
									singkat.
								</p>
							</div>
							...
							<CardContent className="flex items-center justify-between gap-4">
								<div>
									...
									<div className="text-xl font-semibold capitalize">
										{dailyChallenge === "memory" &&
											"Memori"}
										{dailyChallenge === "spatial" &&
											"Spasial"}
										{dailyChallenge === "numerical" &&
											"Numerik"}
									</div>
									<div className="text-sm text-muted-foreground">
										Selesaikan sekali untuk bonus XP ganda.
									</div>
								</div>
								<Button
						...
						<Card>
							...
							<CardContent>
								<div className="text-sm text-muted-foreground mb-3">
									Bangun sesi cepat sesuai waktu luangmu.
								</div>
								...
								<div className="flex items-center justify-between mb-4">
									<div className="flex items-center gap-2">
										...
										<div>
											...
											<div className="text-xs text-muted-foreground">
												Kesulitan menyesuaikan performa
											</div>
										</div>
									</div>
									<Tooltip>
										...
										<TooltipContent side="left">
											Pilih tantangan otomatis berdasar
											progres level.
										</TooltipContent>
									</Tooltip>
								</div>
								<Button className="w-full" onClick={quickStart}>
									Mulai Quick Session
								</Button>
								...
								{import.meta?.env?.DEV && (
									<Button
										...
										onClick={() => handleComplete(20, true)}
									>
						...
						{challenges.map((c) => {
							...
							return (
								...
								>
									<CardHeader>
										<div
											className={`w-12 h-12 rounded-lg bg-gradient-to-r ${c.color} flex items-center justify-center mb-4`}
										>
									...
									<CardContent>
										...
										<div className="flex gap-2">
											<Button
												...
												className="flex-1 bg-gradient-primary hover:bg-gradient-primary/90 group-hover:scale-105 transition-transform"
											>
												Mulai Tantangan
											</Button>
											<Button
				...
				<Dialog
					open={!!showPreviewFor}
					onOpenChange={() => setShowPreviewFor(null)}
				>
					<DialogContent>
						{showPreviewFor && (
							<>
								<DialogHeader>
									<DialogTitle>
										{PREVIEWS[showPreviewFor].title}
									</DialogTitle>
									<DialogDescription>
										{PREVIEWS[showPreviewFor].hint}
									</DialogDescription>
								</DialogHeader>
								<div className="p-4 rounded-md border bg-muted/30 text-sm">
									{PREVIEWS[showPreviewFor].sample}
								</div>
								<div className="flex justify-end">
									<Button
										onClick={() => {
											setSelected(showPreviewFor);
